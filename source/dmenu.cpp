...
INIT_FUNCTION_PTR(GetSpawnerNativeFunc);
INIT_FUNCTION_PTR(EntitySpawnerFunc);

typedef uint32_t U32;

constexpr U32 g_crc32Table[] = {
...
constexpr U32 StringIdHashConcat(U32 base, const char* str)
{
  return (*str) ? StringIdHashConcat(g_crc32Table[(base >> 24) ^ *str] ^ (base << 0x8), str+1) : base;  
}

constexpr U32 StringIdHash(const char* str)
{
...
bool DMenu_Test_Func(uint64_t DMenuStructure, enum DMenu::Message Message)
{
    if (Message == DMenu::Message::OnExecute)
    {
        ...
        uint32_t ArgTest = StringIdHash(spawner_name);
        uint64_t* ret = nullptr;
        // arg, *arg, *unknown
        ret = GetSpawnerNativeFunc(1, (void*)&ArgTest, &ret);
        if (ret)
        {
            uint64_t spawner_ret = EntitySpawnerFunc(ret, nullptr, nullptr, true, 0, 0);
            if (spawner_ret)
            {
                char notify_msg[128];
                snprintf(notify_msg, sizeof(notify_msg), "Spawned %s (#%08x)\n0x%p", spawner_name, ArgTest, ret);
                NotifyStatic(TEX_ICON_SYSTEM, notify_msg);
            }
        }
        else
            return DMenu::FuncReturnCode::FunctionFailure;
